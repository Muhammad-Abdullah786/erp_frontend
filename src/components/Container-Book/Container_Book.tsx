...
const Container_Book = () => {
  ...
  const {
    ...
    mode: "onChange", // To validate fields as they are changed
  });
  ...
  const handleNext = async () => {
    const isValidStep = await trigger(); // Trigger validation for current step
    if (isValidStep) {
      setStep(step + 1);
    }
  };
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onsubmit)}>
            ...
            {step === 2 && (
              <div>
                ...
                <div>
                  ...
                  <Input
                    id="sender_details.phone"
                    placeholder="Sender's Phone. e.g "
                    type="number"
                    {...register("sender_details.phone", {
                      required: "Sender's phone is required",
                      pattern: {
                        value: /^[0-9]{12}$/, // Regex for exactly 10 digits
                        message: "Phone number must be exactly 12 digits",
                      },
                    })}
                  />
                  {errors.sender_details?.phone && (
                    <p className="text-red-500 text-sm">
                      {errors.sender_details?.phone?.message}
                    </p>
                  )}
                </div>
            ...
            {step === 4 && (
              <div>
                ...
                <Button type="submit" disabled={payment_loading || !isValid}>
                  {payment_loading ? (
                    <Loader2 className="animate-spin" />
                  ) : (
                    "Book Now"
